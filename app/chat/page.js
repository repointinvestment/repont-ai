...
export default function ChatPage() {
  ...
  const sendMessage = async () => {
    ...
    const res = await fetch('/api/chat', {
      ...
      body: JSON.stringify({ messages: newMessages.filter(m => m.role !== 'assistant' || messages.indexOf(m) > 0) })
    })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#f5f5f5' }}>
      {/* Header */}
      <div style={{ background: '#0f3460', color: 'white', padding: '14px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.2)' }}>
        <div>
      ...
      {/* Input */}
      <div style={{ background: 'white', padding: '16px 24px', boxShadow: '0 -2px 8px rgba(0,0,0,0.06)' }}>
        <div style={{ display: 'flex', gap: 10, maxWidth: 900, margin: '0 auto' }}>
          ...
          <button
            ...
            style={{ padding: '0 24px', background: loading || !input.trim() ? '#ccc' : '#0f3460', color: 'white', border: 'none', borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: loading || !input.trim() ? 'not-allowed' : 'pointer' }}
          >
            전송
          </button>
        </div>
