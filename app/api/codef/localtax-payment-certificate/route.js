...
//   3) 다건요청(SSO 세션 공유)은 "같은 기관" 로그인 세션 안에서만 됨 — 이 문서는 정부24 로그인이고
//      나머지 문서(사업자등록증명 등)는 국세청 홈택스 로그인이라, 같이 선택해도 인증이 항상
//      각각 따로 뜸(실제 테스트로 확인됨). 그래서 프론트(CodefDocumentIssuance)에서 항상 단독 조회로만
//      선택되도록 강제(soloOnly).
