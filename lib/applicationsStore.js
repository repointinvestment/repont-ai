...
// consultantUsername이 있으면 그 컨설턴트 담당 고객 것만(자기 것만 봐야 함 — 메인메뉴 위젯에서 전체
// 컨설턴트 고객 이름이 다 보이던 구멍이 있었음), 없으면(관리자) 전체.
export async function listReapplyReminders({ withinDays = 14, consultantUsername = null } = {}) {
  await ensureSchema()
  const rows = consultantUsername
    ? await sql`
        ...
        WHERE a.stage = '부결'
          ...
          AND c.consultant_id = ${consultantUsername}
...
export async function listAwaitingAnnouncement(consultantUsername = null) {
  await ensureSchema()
  return consultantUsername
    ? sql`
        ...
        FROM fund_applications a
        JOIN customers c ON c.id = a.customer_id
        WHERE a.stage = '부결' AND a.reapply_status = 'waiting_for_announcement' AND c.consultant_id = ${consultantUsername}
