...
export function buildVerdict({ analysis, form, fundsByKey = {}, rulesByKey = {}, docFacts = null }) {
  ...
  // 중진공 — 마스터 DB의 kosme_* 자금 criteria를 하나씩 대조해서 어떤 자금에 어떤 근거로 접수 가능한지 판별.
  // 서류·CRM에 없는 항목(대표 나이, 폐업 이력, 수출액, 재해확인증 등)은 "확인 필요" 조건으로 남겨 컨설턴트가 물어보게 함.
  {
    ...
    const isManufLike = /제조|건설|운수|광업/.test(industry)
    const threshold = isManufLike ? th['광업·제조·건설·운수'] : th['기타']
    const isSmallBusiness = employees < threshold // 중진공 기준 소상공인(상시근로자 미만)
    const isFood = /음식|요식|카페|외식/.test(industry)
    const hasClosure = form?.hasBankruptcy === 'yes'
    const hasExport = form?.exportRecord === 'yes'
    const sojinkongHyuksinLoan = num(form?.sojingongLoans?.hyuksin)
    const hasSmartDevice = (form?.smartDevices || []).length > 0

    const kosmeFunds = Object.values(fundsByKey)
      .filter((f) => f.key?.startsWith('kosme_') && f.active !== false)
      .sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0))

    const evaluated = []
    for (const f of kosmeFunds) {
      const c = f.criteria || {}
      const ok = [], no = [], ask = []

      // 소상공인 제외 규칙
      if (c.excludes_small_business && isSmallBusiness) {
        if (f.key === 'kosme_milestone') { /* 아래에서 별도 처리 */ }
        else if (c.small_business_exception_sectors?.length) ask.push(`소상공인 규모(직원 ${employees}명 < ${threshold}명) — ${c.small_business_exception_sectors.join('·')} 해당 시에만 예외`)
        else no.push(`소상공인 규모(직원 ${employees}명 < ${threshold}명) — 원칙 제외`)
      } else if (c.excludes_small_business && !isSmallBusiness) ok.push(`직원 ${employees}명 — 소상공인 규모 초과(${threshold}명↑), 중소기업 대상`)
      else if (c.excludes_small_business === false) ok.push('소상공인도 신청 가능한 자금')

      // 업력
      if (c.max_biz_age_exclusive != null) (bizAge < c.max_biz_age_exclusive ? ok : no).push(`업력 ${bizAge}년 (${c.max_biz_age_exclusive}년 미만 ${bizAge < c.max_biz_age_exclusive ? '충족' : '초과'})`)
      if (c.min_biz_age != null) (bizAge >= c.min_biz_age ? ok : no).push(`업력 ${bizAge}년 (${c.min_biz_age}년 이상 ${bizAge >= c.min_biz_age ? '충족' : '미달'})`)

      // 직원 수 최소
      if (c.min_employees != null) (employees >= c.min_employees ? ok : no).push(`상시근로자 ${employees}명 (${c.min_employees}명 이상 ${employees >= c.min_employees ? '충족' : '미달'})`)
      if (c.min_employees_by_industry) {
        const need = isManufLike ? c.min_employees_by_industry['광업·제조·건설·운수'] : c.min_employees_by_industry['기타']
        ;(employees >= need ? ok : no).push(`신청 전 3개월 평균 고용 ${employees}명 (${need}명 이상 ${employees >= need ? '충족' : '미달'})`)
      }

      // 폐업 이력 / 수출 / 기술 / 지정 등 — CRM에 있는 것은 대조, 없는 것은 확인 필요
      if (c.requires_closure_history) (hasClosure ? ok : no).push(hasClosure ? '폐업 이력 있음' : '폐업 이력 없음 (재창업 요건 미충족)')
      if (c.export_min_usd != null) (hasExport ? ask : no).push(hasExport ? `수출 실적 있음 — 최근 1년 ${(c.export_min_usd / 10000).toLocaleString()}만불 이상인지 확인` : '수출 실적 없음')
      if (c.export_max_usd_exclusive != null) ask.push(hasExport ? `수출 실적 있음 — ${(c.export_max_usd_exclusive / 10000).toLocaleString()}만불 미만(내수→수출 초기)인지 확인` : '수출 계획·준비 여부 확인 (내수기업의 수출 진출용)')
      if (c.owner_age_max != null) ask.push(`대표 만 ${c.owner_age_max}세 이하 여부 확인`)
      if (c.requires_any_tech_type) ask.push('개발기술(R&D 성공·특허·인증 등) 보유 여부 확인')
      if (c.requires_hardship_reason) ask.push('공고 열거 경영애로 사유 해당 여부 + 매출·영업이익 10%↑ 감소 확인')
      if (c.requires_disaster_certificate) ask.push('재해중소기업 확인증(시장·군수·구청장) 보유 여부')
      if (c.requires_sojinkong_graduate) {
        if (sojinkongHyuksinLoan > 0 || hasSmartDevice) ask.push('소진공 혁신성장촉진자금 지원 이력(25~26년) 확인 — 있으면 마일스톤 후보')
        else no.push('소진공 혁신성장촉진자금·강한소상공인·LIPS 이력 없음')
      }
      if (c.requires_conversion_plan_approval) ask.push('사업전환계획 승인 여부(승인 후 5년 미만)')
      if (c.requires_restructuring_designation) ask.push('진로제시컨설팅 판정 / 캠코 추천 / 선제적 자율구조개선 지원결정 여부')
      if (c.requires_trade_adjustment_designation) ask.push('산업통상자원부 통상변화대응지원기업 지정 여부')
      if (c.requires_crisis_signal_designation) ask.push('협약은행·중진공 위기징후 추천 여부')
      if (c.requires_corporation) ask.push('법인 여부 확인')
      if (c.requires_external_audit) ask.push('외부감사대상 여부 확인')

      // 요식업 — 융자제외 업종은 아니지만 대표 경험상 수령 사례 없음
      if (isFood) ask.push('요식업은 융자제외 업종은 아니지만 실제 수령 사례 거의 없음 — 신중')
      if (taxDelinquent) no.push('체납')

      let status
      if (no.length) status = '불가'
      else if (ask.length) status = '조건부'
      else status = '가능'
      evaluated.push({ fund: f, status, ok, no, ask })
    }

    const possible = evaluated.filter((e) => e.status === '가능')
    const conditional = evaluated.filter((e) => e.status === '조건부')
    const reasons = [], caveats = []
    let status
    if (taxDelinquent) { status = '현재 불가'; reasons.push('체납') }
    else if (possible.length) {
      status = '접수 가능'
      reasons.push(`조건 충족 자금 ${possible.length}개 (아래 카드). 업력 ${bizAge}년 → ${bizAge < 7 ? '창업기(혁신창업사업화)' : '성장기(신성장기반)'} 트랙`)
    } else if (conditional.length) {
      status = '조건부'
      reasons.push(isSmallBusiness
        ? `소상공인 규모(직원 ${employees}명, 중진공 기준 ${threshold}명 미만) — 원칙 제외이지만 예외 트랙 ${conditional.length}개는 확인 후 접수 가능`
        : `규모는 중소기업 대상이지만 추가 확인 항목 있음 (${conditional.length}개 자금)`)
    } else { status = '현재 불가'; reasons.push('현재 입력값으로 맞는 중진공 자금 없음') }
    if (!isSmallBusiness) caveats.push('매출 클수록 유리, 기술력 평가 비중 큼. 제조·IT 위주')
    if (isSmallBusiness && employees > 0 && employees >= threshold - 1) caveats.push(`직원 1명 더 고용하면 소상공인 규모를 벗어나 중진공 본 트랙 대상`)

    institutions.push({
      key: 'kosme', name: '중진공 (중소기업 정책자금)', status, reasons, caveats,
      funds: [...possible, ...conditional].slice(0, 6).map((e) => ({
        name: e.fund.name.replace(/^중진공 /, ''),
        limit: `운전 ${fmtWon(e.fund.limit_operating)}${e.fund.limit_facility ? ` / 시설 ${fmtWon(e.fund.limit_facility)}` : ''}`,
        condition: [...e.ok, ...e.ask.map((a) => `❓ ${a}`)].join(' · '),
        status: e.status,
      })),
      excluded: evaluated.filter((e) => e.status === '불가').map((e) => ({ name: e.fund.name.replace(/^중진공 /, ''), why: e.no.join(', ') })),
    })
  }
