...
// 관리자 전체 보기 — 누가 어떤 자료를 만들었고 문의가 몇 건 왔는지 성과 관리용.
export async function listAllMaterials() {
  await ensureSchema()
  return sql`
    SELECT m.id, m.title, m.consultant_username, m.created_at, m.updated_at,
           a.name AS consultant_name,
           (SELECT COUNT(*)::int FROM material_inquiries mi WHERE mi.material_id = m.id) AS inquiry_count
    FROM materials m
    LEFT JOIN accounts a ON a.username = m.consultant_username
