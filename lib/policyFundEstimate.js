...
export function estimateInstitutionLimits(customer) {
  ...
  if (isFood && !isFranchiseFood) {
    ...
    const crossNote = hasGiboLoan
      ...
      : hasFoundationLoan
        ...
        : '';
    results.push({ key: 'sinbo', name: '신용보증기금(신보)', limit: Math.round(revenue / 4), note: `제조업 매출 3억 이상, 한도=매출÷4${bonusSuffix}${crossNote}`, eligible: true });
    ...
    const crossNote = hasGiboLoan
      ...
      : hasFoundationLoan
        ? ' ⚠ 재단 대출 보유 — 신보가 재단 대출을 대환하며 보증서 재발급하는 경우 있음'
        : '';
    results.push({ key: 'sinbo', name: '신용보증기금(신보)', limit: Math.round(revenue / 6), note: `${base}${bonusSuffix}${crossNote}`, eligible: true });
  } else {
  ...
  if (isFood && !isFranchiseFood) {
    ...
    results.push({
      ...
      name: '기술보증기금(기보)',
      limit: null,
      note: `충족 요건: ${giboCore.join(', ')} — 정확한 한도는 상담 필요${bonusSuffix}${crossNote}`,
      eligible: true,
    });
  } else {
    results.push({
