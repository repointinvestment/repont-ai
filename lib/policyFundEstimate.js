...
const SERVICE_KEYWORDS = ['서비스업', '컨설팅', '용역']; // 도소매와 동일하게 매출÷6 적용
...
export function estimateInstitutionLimits(customer) {
  ...
  // 신보: 요식업은 해당 없음. 도소매·서비스업은 매출 5억 이상이면 매출÷6
  if (isFood) {
    results.push({ key: 'sinbo', name: '신용보증기금(신보)', limit: 0, note: '요식업은 대상 업종 아님' });
  } else if (isWholesaleOrService && revenue >= 50000) {
    results.push({ key: 'sinbo', name: '신용보증기금(신보)', limit: Math.round(revenue / 6), note: '도소매·서비스업 매출 5억 이상, 한도=매출÷6' });
  } else {
    results.push({ key: 'sinbo', name: '신용보증기금(신보)', limit: 0, note: '매출 5억 이상 조건 미충족' });
  }

  // 기보: 요식업은 해당 없음. 제조업 매출 3억 이상이면 매출÷4
  if (isFood) {
    results.push({ key: 'gibo', name: '기술보증기금(기보)', limit: 0, note: '요식업은 대상 업종 아님' });
