...
export async function ensureSchema() {
  ...
  await sql`ALTER TABLE customers ADD COLUMN IF NOT EXISTS public_token TEXT UNIQUE`
  await sql`
    CREATE TABLE IF NOT EXISTS notification_log (
      ...
      message TEXT,
      variables JSONB, -- 알림톡 템플릿에 넣은 변수 그대로 저장 — 재발송 시 그대로 재사용
      fund_name TEXT, -- 재발송 버튼에 표시할 짧은 제목
  ...
  `
  await sql`ALTER TABLE notification_log ADD COLUMN IF NOT EXISTS variables JSONB`
  await sql`ALTER TABLE notification_log ADD COLUMN IF NOT EXISTS fund_name TEXT`
  schemaReady = true
}
...
// 재방문 고객(카톡 "문의하기"로 들어온 사람)을 위한 개인 토큰 — 없으면 새로 발급. 이 토큰으로
// /apply/[아이디]?t=토큰 접속하면 폼 입력 없이 저장된 정보로 바로 결과를 보여줌.
export async function getOrCreatePublicToken(customerId) {
  await ensureSchema()
  const [existing] = await sql`SELECT public_token FROM customers WHERE id = ${customerId}`
  if (existing?.public_token) return existing.public_token
  const crypto = await import('crypto')
  const token = crypto.randomBytes(16).toString('hex')
  await sql`UPDATE customers SET public_token = ${token} WHERE id = ${customerId}`
  return token
}

export async function logNotification({ customerId, channel, message, variables, fundName, status, error, sentBy }) {
  await ensureSchema()
  const [row] = await sql`
    INSERT INTO notification_log (customer_id, channel, message, variables, fund_name, status, error, sent_by)
    VALUES (${customerId}, ${channel}, ${message || null}, ${variables ? JSON.stringify(variables) : null}, ${fundName || null}, ${status}, ${error || null}, ${sentBy || null})
    RETURNING *
  `
  return row
}

export async function getNotificationLogEntry(id) {
  await ensureSchema()
  const [row] = await sql`SELECT * FROM notification_log WHERE id = ${id}`
  return row || null
}

// 컨설턴트 본인 고객 것만 — "내 발송 내역" 화면에서 씀.
export async function listNotificationsForConsultant(consultantUsername) {
  await ensureSchema()
  return sql`
    SELECT nl.*, c.owner_name, c.business_name, c.consultant_id
    FROM notification_log nl
    JOIN customers c ON c.id = nl.customer_id
    WHERE c.consultant_id = ${consultantUsername}
