...
export function analyzePolicyFunds(form, fundsByKey = {}, rulesByKey = {}) {
  ...
  // 업력이 정확히 몇 년인지 애매한 자가진단(고객 미등록) 상황에서는 form.bizAgeOver7('yes'/'no')로
  // 직접 확정한 값을 우선 씀. 사업자등록증명으로 정확한 업력이 계산된 경우(customer 흐름)는 이 값이 안 넘어오므로
  // 기존처럼 숫자 비교로 판단.
  const isOverThreshold = form.bizAgeOver7 != null ? form.bizAgeOver7 === 'yes' : bizAgeNum >= debtCheckFromBizAge;
  if (isOverThreshold) {
  ...
  const canApply = form.taxDelinquent !== 'yes' && employeesNum <= maxEmployees && (!isOverThreshold || remainingCapacity >= 0);
  ...
  } else if (fHyuksinInnov && canApply && hasSmartDevice && !isHyuksin) {
    warnings.push(`⚠️ ${fHyuksinInnov.name} — 수출실적 1천달러 이상 또는 2년 연속 매출 10% 증가 해당 안 됨 → 신청 불가`);
  }
