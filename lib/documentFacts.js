...
function extractRegistration(payload) {
  ...
  if (!it) return null
  // 매출초과차입금 기준(업력 7년)은 개업일이 아니라 "사업자등록일" 기준으로 계산해야 함 —
  // 개업일과 등록일은 다를 수 있어서(신고가 늦어지는 경우 등) 등록일을 우선으로 쓰고, 없을 때만 개업일로 보완.
  const registerDate = parseYmd(it.resRegisterDate) || parseYmd(it.resOpenDate)
  return {
    openDate: registerDate ? registerDate.toISOString().slice(0, 10) : null,
    bizAgeYears: yearsBetween(registerDate),
...
export function compareWithCustomer(facts, customer) {
  ...
  if (facts.registration?.bizAgeYears != null) {
    ...
    out.push({
      field: 'businessAgeYears', label: '업력', crmValue: null, docValue: `${doc}년 (사업자등록일 ${facts.registration.openDate || '-'})`,
