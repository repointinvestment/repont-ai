...
// 최신 확정 세션을 상품별로 1건씩
async function latestConfirmedSessions(customerId) {
  const rows = await sql`
    SELECT DISTINCT ON (product) product, result_payload, created_at
    FROM codef_auth_sessions
    WHERE customer_id = ${customerId} AND status = 'confirmed'
    ORDER BY product, created_at DESC
  `
  const byProduct = {}
  for (const r of rows) byProduct[r.product] = r
  return byProduct
}
...
export async function getDocumentFacts(customerId) {
  const sessions = await latestConfirmedSessions(customerId)
  const facts = { sources: {} }
...
export function compareWithCustomer(facts, customer) {
  ...
  if (facts.registration?.bizAgeYears != null) {
    const crm = customer?.business_age_years != null ? Number(customer.business_age_years) : null
    const doc = facts.registration.bizAgeYears
    out.push({
      field: 'businessAgeYears', label: '업력(년)', crmValue: crm, docValue: doc, docSource: '사업자등록증명 개업일 ' + (facts.registration.openDate || ''),
      status: crm == null ? 'fill' : Math.abs(crm - doc) >= 1 ? 'mismatch' : 'match',
    })
  }
  ...
  if (facts.vat?.annualSales != null) {
    ...
    out.push({
      field: 'revenueAmount', label: `연매출(만원, ${facts.vat.latestFullYear}년)`, crmValue: crm, docValue: doc, docSource: '부가세과세표준증명 (과세+면세 합산)',
    ...
    out.push({
      field: 'revenueAmount', label: `연매출(만원, ${facts.financial.attrYear}년 재무제표)`, crmValue: crm, docValue: doc, docSource: '재무제표 손익계산서 매출액',
