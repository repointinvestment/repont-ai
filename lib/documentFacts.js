...
//
// 서류별로 뽑는 것:
//   사업자등록증명(corporate-registration): 사업자등록일 → 업력, 업태/종목
...
function extractRegistration(payload) {
  ...
  if (!it) return null
  // 개업일과 사업자등록일은 다를 수 있어서(신고가 늦어지는 경우 등) 둘 다 따로 잡아둠.
  // 매출초과차입금 기준(업력 7년)은 사업자등록일 기준으로 계산 — 나머지 용도(개업일 자체가 필요한 자금 조건 등)엔 businessStartDate를 씀.
  const businessStartDate = parseYmd(it.resOpenDate)
  const registrationDate = parseYmd(it.resRegisterDate) || businessStartDate
  return {
    openDate: registrationDate ? registrationDate.toISOString().slice(0, 10) : null, // (구) 이름 유지 — 사업자등록일
    registrationDate: registrationDate ? registrationDate.toISOString().slice(0, 10) : null,
    businessStartDate: businessStartDate ? businessStartDate.toISOString().slice(0, 10) : null,
    bizAgeYears: yearsBetween(registrationDate),
...
export function compareWithCustomer(facts, customer) {
  ...
  if (facts.registration?.bizAgeYears != null) {
    // 업력은 서류(사업자등록일)가 더 정확하므로 비교하지 않고 자동 적용됨(document-facts GET에서 CRM 갱신). 여기선 적용 결과만 알려줌.
    const doc = facts.registration.bizAgeYears
    out.push({
      field: 'businessAgeYears', label: '업력', crmValue: null, docValue: `${doc}년 (사업자등록일 ${facts.registration.registrationDate || '-'} 기준)`,
      docSource: '사업자등록증명 — 자동 적용됨', status: 'applied',
    })
  }
  if (facts.registration?.businessStartDate) {
    // 개업일도 매출초과차입금 계산엔 안 쓰지만(그건 사업자등록일 기준) 별도로 필요한 자금 조건이 있어 따로 저장.
    out.push({
      field: 'openDate', label: '개업일', crmValue: null, docValue: facts.registration.businessStartDate,
      docSource: '사업자등록증명 — 자동 적용됨', status: 'applied',
    })
  }
