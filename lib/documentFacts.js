...
export function compareWithCustomer(facts, customer) {
  ...
  if (facts.businessHistory?.reliable) {
    ...
    const crmBankruptcy = pfd.hasBankruptcy === 'yes' ? true : pfd.hasBankruptcy === 'no' ? false : null
    const docBankruptcy = bh.hasClosureHistory
    const detail = bh.businesses.map((b) => `${b.name}(${b.active ? '활성' : '폐업'})`).join(', ')
    out.push({
      field: 'hasBankruptcy', label: '폐업 이력', crmValue: crmBankruptcy == null ? null : (crmBankruptcy ? '있음' : '없음'),
      docValue: `${docBankruptcy ? '있음' : '없음'} — 사업자 ${bh.totalCount}개 중 활성 ${bh.activeCount}개·폐업 ${bh.closedCount}개 (${detail})`,
      docSource: '사업자등록증명 vs 부가세과세표준증명 사업자 목록 대조',
      status: crmBankruptcy == null ? 'fill' : crmBankruptcy !== docBankruptcy ? 'mismatch' : 'match',
      rawValue: docBankruptcy ? 'yes' : 'no',
    })
    if (bh.activeCount > 0) {
      const crmCount = pfd.currentBizCount || null
      out.push({
        field: 'currentBizCount', label: '현재 사업자 개수', crmValue: crmCount, docValue: String(bh.activeCount),
        docSource: '사업자등록증명(현재 활성 사업자 수)',
        status: !crmCount ? 'fill' : String(crmCount) !== String(bh.activeCount) ? 'mismatch' : 'match',
        rawValue: String(bh.activeCount),
      })
    }
  }
  return out
